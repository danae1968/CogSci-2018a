...
logfileName = ['taskAutomaticity_acrossExpAnalysis_' ...
...
if exist(filePath, 'file') == 2 && ~overwrite    % if log file exists load it
    ...
    for corr_cond_idx = 1:length(corr_values) 
        ...
        for rep = 1:numReps
            ...
            taskAutomaticityExp1 = nan(1, numSubj);
            taskAutomaticityExp2 = nan(1, numSubj);
            for subj = 1:numSubj
                [Za, Zb] = randomcorrelatednum(correlation);
                taskAutomaticityExp1(subj) = Za * taskAutomaticity_scale + default_taskAutomaticity;
                taskAutomaticityExp2(subj) = Zb * taskAutomaticity_scale + default_taskAutomaticity;
            end
            ...
            for subj = 1:numSubj
                ...
                taskAutomaticity = taskAutomaticityExp1(subj);
                ...
                % vary motivational parameter
                taskAutomaticity = taskAutomaticityExp2(subj);
                agent.outcomeProbabilityFnc = @(u) 1./(1+exp(-default_controlEfficacy*u - taskAutomaticity));
